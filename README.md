...
|sys/ioctl.h|ioctl|Manipulates the underlying device parameter of special files|
|sys/ioctl.h|fstat|is identical to stat but using file descriptors|
|sys/ioctl.h|lstat|is identical to stat but for pathnames that are symbolink link|
|sys/ioctl.h|stat|retrieve information of a file referenced by its pathname in a struct of type stat. after calling stat in the stat.st_size has total size in bytes|
...
|unistd.h|getenv|Searches the environment list and returns a pointer to the value of variable passed or NULL if ther is not match|
